...
* @license MIT
*/
!function(window,document){"use scrict";function defineBootmark(){function _insertLinkTag(t){if(!window.$('link[href="'+t+'"]').length){var n=window.$("<link />");n.attr({href:t,rel:"stylesheet"}),window.$("head").append(n)}}function _insertMetaTag(t,n){if(!window.$('meta[content="'+n+'"]').length){var e=window.$("<meta />");e.attr({name:t,content:n}),window.$("head").append(e)}}function _insertHtml(t,n,e){return new Promise(function(o){var i=new window.showdown.Converter(n.showdown).makeHtml(e);if(n.html.toc?n.template='<div class="container-fluid" id="'+n.html.tocTitle.replace(/ /g,"-")+'"><div class="row"><div class="col-sm-3 col-md-3 col-lg-2"><nav class="navbar navbar-default navbar-fixed-side"><div class="container"><div class="navbar-header"><button class="navbar-toggle" data-target="#nav" data-toggle="collapse"><span class="sr-only">Toggle navigation</span><span class="icon-bar"></span><span class="icon-bar"></span><span class="icon-bar"></span></button><a class="navbar-brand active page-scroll" href="#'+n.html.tocTitle.replace(/ /g,"-")+'">'+n.html.tocTitle+'</a></div><div class="collapse navbar-collapse" id="nav"><ul class="bootmark-toc nav navbar-nav"></ul></div></div></nav></div><div class="bootmark-main bootmark-toc col-sm-9 col-md-9 col-lg-10">'+i+"</div></div></div>":n.template='<div class="container"><div class="row"><div class="bootmark-main">'+i+"</div></div></div>",t.html(n.template),n.html.toc&&window.$("h1, h2, h3, h4, h5, h6",t).each(function(n,e){window.$(".bootmark-toc.nav.navbar-nav",t).append(window.$("<li></li>").addClass("bootmark-"+e.localName).html(' <a class="page-scroll" href="#'+e.id+'">'+e.innerText+"</a>"))}),window.$("table",t).length&&window.$("table",t).addClass(" table table-striped table-bordered ").wrap(' <div class="table-responsive"></div> '),n.html.indent&&window.$("p",t).length&&window.$("p",t).addClass("bootmark-indent"),n.html.credit&&!window.$("#bootmark-footer").length){var a='<div class="container-fluid bg-primary"><p class="text-center"><a style="color: inherit" href="https://obedm503.github.io/bootmark">This project uses bootmark. Bootmark allows developers to focus on their projects and not how they are presented.</a><p></div>',s=window.$('<footer id="bootmark-footer"></footer>').html(a);window.$("body").append(s)}n.showdown.extensions.indexOf("prettify")>=0&&window.prettyPrint(),o(i)})}var defaults={markdown:!1,fetch:!1,join:"----",css:"https://obedm503.github.io/bootmark/dist/bootmark.min.css",promise:!1,html:{indent:!1,toc:!0,tocTitle:window.$(document).attr("title"),theme:"readable",prettifyTheme:"atelier-forest-light",prettify:!0,credit:!0},showdown:{parseImgDimensions:!0,simplifiedAutoLink:!0,literalMidWordUnderscores:!0,strikethrough:!0,tables:!0,tablesHeaderId:!0,tasklists:!0,extensions:[]}};window.$.fn.bootmark=function(options){var element=this;if("string"==typeof options.html||"string"==typeof options.showdown||"string"==typeof options.fetch)for(var i in options)"string"==typeof options[i]&&("html"===i||"showdown"===i||"fetch"===i&&"["===options[i].trim()[0])&&options.hasOwnProperty(i)&&(options[i]=eval("("+options[i]+")"));var config=window.$.extend(!0,{},defaults,window.$.fn.bootmark.options||{},options);config.html.prettify&&config.showdown.extensions.indexOf("prettify")<0&&config.showdown.extensions.push("prettify"),element.hide(),_insertMetaTag("viewport","width=device-width, initial-scale=1"),_insertLinkTag("https://maxcdn.bootstrapcdn.com/bootswatch/3.3.7/"+config.html.theme.toLowerCase()+"/bootstrap.min.css"),_insertLinkTag(config.css),config.showdown.extensions.indexOf("prettify")>=0&&_insertLinkTag("https://jmblog.github.io/color-themes-for-google-code-prettify/themes/"+config.html.prettifyTheme+".min.css","bootmark-prettify-theme");var inserted;if(config.markdown)inserted=_insertHtml(element,config,config.text);else if(config.fetch)if("string"==typeof config.fetch)inserted=window.fetch(config.fetch).then(function(t){return t.text()}).then(function(t){return _insertHtml(element,config,t)});else{var fetches=config.fetch.map(function(t){return window.fetch(t).then(function(t){return t.text()})});inserted=window.Promise.all(fetches).then(function(t){var n=t.join("\n\n"+config.join+"\n\n\n");return _insertHtml(element,config,n)})}else inserted=_insertHtml(element,config,element.text());return config.promise?inserted.then(function(t){return element.show(),t},function(){element.show()}):(inserted.then(function(){element.show()},function(){element.show()}),element)},window.$(function(){window.$(document).on("click","a.page-scroll",function(t){t.preventDefault(),window.$("html, body").animate({scrollTop:window.$(window.$.attr(this,"href")).offset().top},900)}),window.$("#nav a").click(function(){window.$("#nav").collapse("hide")}),window.$("bootmark").length&&window.$("bootmark").each(function(){var t=window.$(this);t.bootmark({fetch:t.attr("fetch"),join:t.attr("join"),html:t.attr("html"),css:t.attr("css"),showdown:t.attr("showdown"),promise:t.attr("promise")})}),window.$(".bootmark").length&&window.$(".bootmark").each(function(){var t=window.$(this);t.bootmark({fetch:t.attr("data-fetch"),join:t.attr("data-join"),html:t.attr("data-html"),css:t.attr("data-css"),showdown:t.attr("data-showdown"),promise:t.attr("data-promise")})})})}if("undefined"==typeof window.$)throw new Error("JQuery is not defined");if("undefined"==typeof window.showdown)throw new Error("Showdown is not defined");if("undefined"==typeof window.$.fn.modal)throw new Error("Bootstrap is not defined");"undefined"==typeof window.prettyPrint&&console.warn("Code Prettify is not defined"),"undefined"==typeof window.Promise&&console.warn("Promise API is not defined"),"undefined"==typeof window.fetch&&console.warn("fetch API is not defined"),"undefined"==typeof window.$.fn.bootmark&&defineBootmark()}(window,document);